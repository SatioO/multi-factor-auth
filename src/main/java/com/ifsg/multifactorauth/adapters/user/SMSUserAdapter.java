...
@Service
public class SMSUserAdapter implements IUserAdapter {
    ...
    public SMSUserAdapter(UserRepository userRepository, OTPPolicyConfig otpPolicyConfig, SoftTokenRepository softTokenRepository, IOTPGeneratorAdapter IOTPGeneratorAdapter) {
    ...
    public Boolean assignToken(AssignTokenBodyDTO data) {
        return userRepository
                .findByExternalId(data.getExternalId()).map(foundUser -> {
                    softTokenRepository
                            .findByExternalIdAndType(PageRequest.of(0,1), data.getExternalId(), TokenType.SMS)
                            .stream().findFirst()
